...
NEVENTS="-e ${NPER}"      # -n XXXX number of events, -e XE16 for POT

TOPVOL="volArgonCubeActive"

TOPDIR="/pnfs/dune/persistent/users/marshalc/nd_magnets"
...
${CP} ${TOPDIR}/inputs/gevgen.tar.gz gevgen.tar.gz
...
${CP} ${TOPDIR}/inputs/edep-sim.tar.gz ${PWD}/edep-sim.tar.gz
...
# The MakeProject in dumpTree.py won't work if edep-sim is in the library path
# This is a hack
...
# The G4 file is big, so only copy the ntuple
#${CP} ${MODE}.${RNDSEED}.ghep.root ${TOPDIR}/outputs/ghep/${GEOMETRY}/${MODE}.${RNDSEED}.ghep.root
#${CP} edep.${RNDSEED}.root ${TOPDIR}/outputs/edep/${GEOMETRY}/${MODE}.${RNDSEED}.edepsim.root
